...
from python_common.web.internet_protocols import default_scheme_for_port
...
def is_spider_trap(url):
    # There is a high probability that the URL is a spider trap if
    # there are either more than 256 characters or more than 20 slashes in the path portion.
    path_portion = urlsplit(url)[2]
    if len(path_portion) > 512 or path_portion.count('/') > 30:
        return True


def remove_http_and_https_scheme(url):
...
def remove_url_scheme(url):
    split_point = url.find('://', 0, 16)
    return url[split_point + 3:] if split_point >= 0 else url


def extract_scheme(url):
    split_point = url.find('://', 0, 16)
    return url[: split_point] if split_point > 0 else ''


def is_index_html(url):
    split = urlsplit(url)
    return (not split.query) and ('.' not in split.path.split('/')[-1])
...
def fix_missing_scheme(url, urlparts):
    if not urlparts[1]:
        if '://' in urlparts[2][:10]:
            return urlparse.urlsplit(url[len(urlparts[0]) + 1:])
        if '//' in urlparts[2]:
            parts = urlparts[2].split('//', 1)
            if len(parts) > 1 and len(parts[0]) < 10:
                return urlparse.urlsplit(parts[0] + '://' + url[len(parts[0]) + 2:])

        urlparts[2] = urlparts[0] + ':' + urlparts[2]

    parts = urlparts[2].split('/', 1)
    m = netloc_port_pattern.search(parts[0])
    if m:
        if m.group(1) in default_scheme_for_port:
            scheme = default_scheme_for_port[m.group(1)]
            urlparts = urlparse.urlsplit(scheme + '://' + url.replace(m.group(0), ''))
        else:
            if '.' not in parts[0]:
                raise InvalidUrl(url)
            urlparts = urlparse.urlsplit('http://' + url)
...
http_scheme_pattern = re.compile(r'[a-zA-Z][a-zA-Z0-9+\-.]+$')
